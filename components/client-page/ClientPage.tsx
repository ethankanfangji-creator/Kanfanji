...
import { takeInputFiles, validateImportedMedia, mapMediaImportErrorCode } from "@/lib/media-import";
import {
...
} from "@/lib/viewing-wizard/question-answers";
import { decideStepNavigationRequest } from "@/lib/viewing-wizard/step-navigation";
import { useBillingEntitlement } from "@/components/client-page/useBillingEntitlement";
import {
  createStableLocalSessionId as mintStableLocalSessionId,
  resolveExistingLocalSessionId as resolveExistingLocalSessionIdHelper,
} from "@/components/client-page/local-session-id";
import { WizardHeaderBilling } from "@/components/client-page/WizardHeaderBilling";
import { PaywallDialog } from "@/components/client-page/PaywallDialog";
import {
  decideCaptureStart,
  hasCaptureExplained,
...
export function ClientPage() {
  ...
  const onBillingStatusMessage = useCallback((message: string) => {
  ...
  const {
    ...
    user,
    paywall: messages.paywall,
    t,
    onStatusMessage: onBillingStatusMessage,
  });
  ...
  async function resolveExistingLocalSessionId(): Promise<string | null> {
    const id = await resolveExistingLocalSessionIdHelper({
    ...
    if (id) draftSessionIdRef.current = id;
    return id;
  }
  ...
  async function createStableLocalSessionId(): Promise<string> {
    ...
    return mintStableLocalSessionId({
      createEntityId,
  ...
  function activeQuestionBank() {
    return activeQuestionBankOrDefault(
      questions,
      ensureDefaultFieldQuestions([], messages.fieldChecklist.labels),
    );
  }
  ...
  function applyQuestionAnswer(
    ...
    setQuestions((current) => {
      const next = applyAnswerToQuestions(current, questionId, answer, preview);
  ...
  async function processRecording(
    ...
    if (pendingId != null) {
      setQuestions((current) => markQuestionAnalyzing(current, pendingId));
    }
    ...
    try {
      ...
      setQuestions((current) => {
        ...
        nextQuestions = merged.map((question) =>
          resolvePendingAnalyzingQuestion(
            question,
            pendingId,
            messages.bank.captureAudioSummary,
          ),
        );
      ...
      setLiveMarkers([]);
      if (pendingId != null) {
        setQuestions((current) => clearAnalyzingStatus(current, pendingId));
        clearPendingAnswerQuestion();
        scrollToQuestionCard(pendingId);
      }
  ...
  function mediaImportError(code: string): string {
    return mapMediaImportErrorCode(code, messages.mediaImport);
  }
  ...
  async function onImportAudio(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    event.target.value = "";
    if (!file) return;
    if (isAudioCaptureBusy(audioState, captureLockRef.current)) {
      setSyncMessage(messages.permissions.busyElsewhere);
      return;
    }
  ...
  async function locateWithDeviceGps() {
    setLocationError("");
  ...
  function requestGoToStep(target: WizardStep) {
    const decision = decideStepNavigationRequest({
      from: wizardStep,
      to: target,
      openHighPriorityCount: countHighPriorityOpenTickets(questions),
    });
    if (decision.kind === "noop") return;
    if (decision.kind === "confirm-back-to-address") {
      setShowBackToAddressConfirm(true);
      return;
    }
    if (decision.kind === "confirm-high-priority") {
      setShowHighPriorityConfirm(true);
      return;
    }
    void goToStep(decision.target);
  }
      ...
      <div className={`page-container pt-[max(24px,env(safe-area-inset-top))] ${
        ...
        <WizardHeaderBilling
          copy={{
            ...
            brandSubtitle: messages.brand.subtitle,
            freeQuota: messages.status.freeQuota,
            manage: messages.paywall.manage,
            manageLoading: messages.paywall.manageLoading,
            sync: messages.paywall.sync,
            syncLoading: messages.paywall.syncLoading,
            records: messages.nav.records,
            loggedIn: messages.status.loggedIn,
            guest: messages.status.guest,
            noKeys: messages.status.noKeys,
          }}
          t={t}
          user={user}
          isPro={isPro}
          ...
          billingSyncLoading={billingSyncLoading}
          configured={configured}
          onOpenPortal={() => void openBillingPortal()}
          onResync={() => void resyncBilling()}
        />
        ...
        {draftReady && wizardStep === 1 && (
          <StepSetup
            ...
            onUseMyLocation={() => void locateWithDeviceGps()}
            locating={locating}
...
<div className="bg-white rounded-[24px] border border-black/[0.05] shadow-[0_4px_20px_rgba(0,0,0,0.04)] p-5 mb-4 relative overflow-hidden">
        ...
        <PaywallDialog
          ...
          copy={{
            ...
            body: messages.paywall.body,
            close: messages.card.close,
            perMonth: messages.paywall.perMonth,
            feature1: messages.paywall.feature1,
            feature2: messages.paywall.feature2,
            feature3: messages.paywall.feature3,
            loading: messages.paywall.loading,
            cta: messages.paywall.cta,
            manage: messages.paywall.manage,
            manageLoading: messages.paywall.manageLoading,
            sync: messages.paywall.sync,
            syncLoading: messages.paywall.syncLoading,
            footer: messages.paywall.footer,
          }}
          hasStripeCustomer={hasStripeCustomer}
          checkoutTimedOut={checkoutTimedOut}
